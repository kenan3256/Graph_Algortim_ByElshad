...
public final class Johnson {
        ...
        graph.getVertices()
                .forEach(srcVertex -> {
                    ...
                    for (int i = 0; i < minDistances.size(); i++) {
                        ...
                        result[srcVertex.getIdx()][i] = (minDistances.get(i) == GraphConst.INFINITY) ? GraphConst.INFINITY
    ...
    private static Vertex addNewNode(DirectedGraph graph) {
        ...
        graph.getVertices().stream()
                ...
                .forEach(vertex -> graph.addEdge(newNode.getIdx(), vertex.getIdx(), GraphConst.ZERO));
        return newNode;
    }
